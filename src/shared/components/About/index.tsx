import * as C from "./style";

export const About = () => {
  return (
    <C.Container id="HOME">
      <div>
        <p>
          OLÁ ME CHAMO MARCONDES, TENHO 28 ANOS, SOU DE FORTALEZA-CE E FORMADO EM ANÁLISE E DESENVOLVIMENTO DE SISTEMAS PELA UNIVERSIDADE ESTÁCIO DE SÁ.
        </p> <p>
          COMECEI MINHA CARREIRA NO ANO DE 2020, QUANTO ATRAVES DE UM COLEGA CONHECI A PROGRAMAÇÃO, ENTÃO VENHO ME PREPARANDO PARA CONSQUISTAR UMA VAGA NESSA ÁREA DESAFIADORA. </p> <p>
          ESTUDEI C++/PYTHON E PHP NA FACULDADE, MAS FOI COM JAVASCRIPT QUE ME ENCONTREI, TANTO PELAS FACILIDADES, QUANTO PELAS POSSIBILIDADES.
        </p><p>
          PERCEBI QUE O FRAMEWORK REACT VINHA CRESCENDO MUITO NO MERCADO E APOSTEI NELA, E VENHO ESTUDANDO, E APRIMORANDO MINHAS SKILLS.</p> <p>
          JA PASSEI POR UME ESTÁGIO NO TRIBUNAL ELEITORAL DA MINHA CIDADE, E PRESTEI ASSESSORIA PARA ALGUNS COLEGAS DA FACULDADE, ALÉM DE PRESTAR SERVIÇO FREELA PARA ALGUMAS EMPRESAS.
        </p><p>
          HOJE MEU FOCO É MRE PREPARAR PARA CONQUISTAR UM PERFIL FULLSTACK.
        </p>
          
      </div>
    </C.Container>
  );
};

/**
 *        Olá me chamo Marcondes, tenho 28 anos, sou de
          Fortaleza-CE e formado em Análise e desenvolvimento de sistemas 
          pela a universidade Estácio de Sá.
          Comecei minha carreira no ano de 2020, quando atravez de um colega conheci a programação, 
          então venho me preparar para conquistar uma vaga nessa área desafiadora.
          Estudei C++/Python e PHP na faculdade, mas foi com JavaScript que eu me encontrei, tanto pelas
          facilidade, quanto pelas possibilidades.
          Percebi que o Framework React vinha crescendo muito no mercado e apostei nela, e venho estudando,
           e aprimorando minhas skills.
          Já passei por um estágio no Tribual Eleitoral da minha cidade, e prestei
          assessoria para alguns colegas da faculdade, alem de prestar serviço Freela para algumas empresas.
          Hoje meu foco é me preparar para consquistar um perfil FullStack.



          Hello my name is Marcondes, I'm 28 years old, I'm from
          Fortaleza-CE and graduated in Systems Analysis and Development 
          by the university Estácio de Sá.
          I started my career in the year 2020, when through a colleague I met programming, 
          so I come to prepare myself to win a place in this challenging area.
          I studied C++/Python and PHP in college, but it was with JavaScript that I found myself, both for the
          ease, as for the possibilities.
          I realized that the React Framework had been growing a lot in the market and I bet on it, and I've been studying,
           and honing my skills.
      I've been through an internship at the Electoral Court of my city, and I've been there.
          advice for some college colleagues, in addition to providing Freela service to some companies.
          Today my focus is to prepare myself to achieve a FullStack profile.
       
       
          
 */


/**
 * <h3>Soft Skills</h3>
<p>
  Facilidade de adaptação – Organização – Responsabilidade – Agilidade
  com Qualidade – Fácil Aprendizagem – Boa Comunicação
</p>
<h3>Conhecimentos</h3>
<p>SCRUM, SOLID, KANBAN, Desenvolvimento Ágil</p>
<h3>Idioma</h3>
<p>Inglês Técnico</p>
<h3>Objetivo</h3>
<p>
  Busco conseguir uma oportunidade para por em prática e desenvolver
  todo o conhecimento que adquiri ao longo dos anos, contribuindo,
  aprendendo e crescendo junto com a empresa.
</p>
 * 
 */
